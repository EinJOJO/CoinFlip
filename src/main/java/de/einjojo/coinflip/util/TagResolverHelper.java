...
public class TagResolverHelper {
    ...
    public static TagResolver[] createRequestResolver(GameRequest gameRequest) {
        return new TagResolver[]{Placeholder.component("anti_bet", gameRequest.getBet() == GameResult.HEAD ? MessageKey.TAIL.getComponent() : MessageKey.HEAD.getComponent()), createBetPlaceholder(gameRequest.getBet()), Placeholder.unparsed("player", gameRequest.getRequesterPlayer().getName()), Placeholder.unparsed("money", String.valueOf(gameRequest.getMoney())), Placeholder.unparsed("amount", String.valueOf(gameRequest.getMoney()))

        };
    }

    public static TagResolver[] createHistoryResolver(GameHistory history) {
        String name = Optional.of(Bukkit.getOfflinePlayer(history.getUuid())).map(OfflinePlayer::getName).orElse("unknown");
    ...
    public static TagResolver[] createSubCommandResolver(SubCommand subCommand) {
        ...
        return new TagResolver[]{Placeholder.unparsed("command", subCommand.getCommand()), Placeholder.unparsed("description", subCommand.getDescription()), Placeholder.unparsed("permission", permission), Placeholder.unparsed("syntax", subCommand.getSyntaxSuggestion()), Placeholder.unparsed("alias", alias),};
    }
